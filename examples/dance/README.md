...
2. **Raccoon dance** — `raccoon/animation.pup`, 21,940 bytes, a lossless PUPZ wrapper around the 33,070-byte PUP1 compatibility file. One 4.332-second dance, continuous rig deformation and original SVG colors.
...
The latest fox release changes four pose selections (F021 → F004, F035 →
F019, F037 → F004, F052 → F020), preserving the geometry banks and file size.
The raccoon uses the selected vertical wrist mask and an additional raised
pose derived from its existing F8 curves. Its hidden shoulder links overlap
the head and hand to prevent the F12 gap; head/body/leg/tail outlines and
motion are unchanged. The editable snapshots reproduce these updated files.
