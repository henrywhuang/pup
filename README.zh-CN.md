...
下载区提供八个动画的独立下载及 ZIP。小黄鸟转身使用 `.puc`，并提供骨骼映射下载；
ZIP 同时包含骨骼映射和浣熊舞蹈的 PUP1 兼容文件。答题文件包含答对、答错两个动作。

打开 **Rig / bindings** 可以查看实际骨骼映射、曲线控制点、变形权重和变换节点。
每个案例都提供绑定 JSON 下载，ZIP 也包含这些数据。狐狸跳舞采用矢量姿态切换，
页面会明确说明没有骨骼绑定，不会虚构骨骼。详见[检查器说明](docs/rig-inspection.md)。

## 动画案例与真实对照

- **前两个案例：狐狸跳舞、浣熊跳舞**，原 WebP 在左、PUP 在右，发布文件逐字节保留。
- **第三个案例：小鸟跳舞**，原始 97 帧 WebP 对照 9,575 字节的 PUP，时长 4.033 秒。
- **第四个案例：小黄鸟转身**，左侧切换原始七个 SVG 视图，右侧播放转身；Canvas／SVG 均可显示原有 8 个骨骼。
...
狐狸舞蹈为 **85,183 B**，浣熊舞蹈为 **22,222 B**，小鸟舞蹈为 **9,575 B**，小黄鸟转身 PUC 为 **68,136 B**。
